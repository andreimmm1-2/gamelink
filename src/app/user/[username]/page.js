    ...
    return (
      <main className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 flex items-center justify-center">
        <div className="text-center">
          ...
          <Link href="/discover" className="px-4 py-2 bg-slate-700 hover:bg-slate-600 text-white rounded-lg transition">
            Back to Discovery
          </Link>
        </div>
      </main>
  ...
  return (
    <main className="min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900">
      ...
      <div className="max-w-4xl mx-auto px-4 pt-6">
        <Link href="/discover" className="text-slate-400 hover:text-slate-300 text-sm mb-6 inline-block">
          ← Back to Discovery
        </Link>
      </div>
      ...
      <div className="max-w-4xl mx-auto px-4 py-8">
        ...
        <div className="grid grid-cols-3 gap-4 mt-6">
          <div className="bg-slate-800 rounded-lg p-4 border border-slate-600 border-opacity-30 text-center">
            <div className="text-2xl font-bold text-slate-400">{user.profiles?.length || 0}</div>
        ...
        {(user.bio || user.description) && (
          <div className="mt-8 bg-slate-800 rounded-lg p-6 border border-slate-600 border-opacity-30">
              ...
              {Object.entries(profilesByGame).map(([game, profiles]) => (
                <div key={game}>
                  ...
                  <div className="space-y-3">
                    {profiles.map(p => (
                      <div key={p.id || p._id} className="bg-slate-800 rounded-lg p-4 border border-slate-600 border-opacity-20">
                        <h4 className="text-slate-400 font-semibold mb-2">{p.inGameName || p.in_game_name}</h4>
                        {p.description && (
        ...
        {/* Empty State */}
        {(!user.profiles || user.profiles.length === 0) && (
          <div className="mt-8 bg-slate-800 rounded-lg p-8 border border-slate-600 border-opacity-30 text-center">
