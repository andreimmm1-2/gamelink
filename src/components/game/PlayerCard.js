  ...
  return (
    <Card>
      <div className="flex items-start">
        <img src={user.profilePicture || '/images/avatars/default.png'} alt="avatar" className="w-12 h-12 rounded-full mr-3 object-cover" />
        <div className="flex-1">
          <div className="flex justify-between items-baseline">
            <div>
              <Link href={`/user/${user.username}`} className="font-semibold hover:underline">{user.username}</Link>
              <div className="text-sm text-gray-300">{profile.inGameName}</div>
            </div>
